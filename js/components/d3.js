var DT_d3 = (function () {
	...
	function d3GetLineColor(me, seriesIndex) {
		...
		if ($.isArray(me.block.lineColors) && me.block.lineColors[seriesIndex]) {
		...
		if (seriesIndex === 1 && typeof me.block.secondaryAccentColor !== 'undefined') {
	...
	function d3FormatCurrentValue(device, unitsOverride) {
		...
		var unit = typeof unitsOverride !== 'undefined' ? unitsOverride : device.txtUnit
	...
	function d3GetChartValueKey(me, seriesIndex) {
		if ($.isArray(me.block.chartValues) && typeof me.block.chartValues[seriesIndex] !== 'undefined') {
	...
	me.block.range = "day";
		...
		$.each(me.deviceIdxs, function (_, idx) {
			...
			if (!device) {
				console.warn('D3 component: Device not found (idx: ' + idx + ')')
				return
			}
	...
	function d3FormatData(me, seriesResults) {
		...
		$.each(seriesResults, function (seriesIndex, seriesResult) {
			...
			if (!seriesResult || typeof seriesResult.result !== 'object' || !seriesResult.result.length) {
				return
			}
