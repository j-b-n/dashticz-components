...
var DT_ha_gauge = (function () {
    ...
    function buildHTML(me, device) {
        ...
        if (me.block.GradientColors) {
            html += '<defs>'
            html += getLinerGradient(
                value,
                me.block.GradientColors,
                me.block.idx,
            )
            html += '</defs>'
        ...
        html +=
            '<path d="M -40 0 A 40 40 0 0 1 40 0" style="fill:none;stroke:' +
            color +
            ';stroke-width:10" />'
        ...
        html +=
            '<path class="ha-gauge-arc-background" d="M ' +
            cos +
            ' ' +
            sin +
            ' A 40 40 0 0 1 40 0" />'
...
*/
        ...
        if (typeof me.block.segments !== 'undefined') {
            me.block.segments.forEach((element) => {
                ...
                html +=
                    '<path class="level" d="M ' +
                    cos +
                    ' ' +
                    sin +
                    ' A 40 40 0 0 1 40 0" style="fill:none;stroke:' +
                    element[1] +
                    ';stroke-width:10" />'
            })
        }
        ...
        if (me.block.needle) {
            ...
            html +=
                '<path class="ha-gauge-needle" d="M -25 -2.5 L -47.5 0 L -25 2.5 z" transform="rotate(' +
                angle +
                ')" style="fill:' +
                color +
                ';stroke:none" />'
        ...
        html +=
            '<text class="ha-gauge-value-text" x="0" y="-5" text-anchor="middle" dominant-baseline="middle" font-size="10px" fill="' +
            color +
            '">'
        html += value
