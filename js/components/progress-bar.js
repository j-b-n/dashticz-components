var DT_progress_bar = (function () {
    function buildHTML(me) {
        ...
        if (me.block.header !== false) {
            headerHTML = '<div id="progress-header-' + me.block.idx + '" class="progress-header"></div>';
        }
        
        var valueHTML = '';
        if (me.block.value && me.block.value !== 'after') {
            // Value in separate div for left/right/below positions
            valueHTML = '<div id="progress-value-' + me.block.idx + '" class="progress-value progress-value-' + me.block.value + '"></div>';
        }
        
        var barHTML = '<div class="progress-container">';
        if (me.block.value === 'inside') {
            barHTML += '<div id="progress-value-' + me.block.idx + '" class="progress-value progress-value-inside"></div>';
        }
        barHTML += '<div id="progress-fill-' + me.block.idx + '" class="progress-fill"></div>' +
            '</div>';
        ...
        return '<div id="progress-bar-' + me.block.idx + '" class="block_' +
            me.block.type +
            ' col-xs-' +
            width +
            '">' +
    ...
    function updateProgress(me, device) {
        ...
        var fillElement = document.getElementById('progress-fill-' + me.block.idx);
        fillElement.style.width = percentage + '%';
        fillElement.style.backgroundColor = me.block.themes[me.block.theme] || me.block.themes.default;
        
        // Update value display if enabled
        if (me.block.value === 'after') {
            // Append value to header
            var headerElement = document.getElementById('progress-header-' + me.block.idx);
            if (headerElement) {
                var headerText = headerElement.dataset.headerText || '';
                headerElement.innerText = headerText + ' ' + Math.round(percentage) + '%';
            }
        } else if (me.block.value) {
            // Update separate value element
            var valueElement = document.getElementById('progress-value-' + me.block.idx);
            if (valueElement) {
