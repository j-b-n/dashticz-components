...
var DT_suncard = (function () {
    ...
    function drawSunPath(svgElementId, percent, solarEvents) {
        ...
        const sunriseHours =
            ...
            sunrise.getSeconds() / 3600
        const sunriseAzimuth = (sunriseHours / 24) * 360
        const sunriseAltitude =
            -maxAltitude * Math.cos((2 * Math.PI * sunriseAzimuth) / 360)
        ...
        const Y_current_on_path =
            centerY - (altitude * MAX_RADIUS) / maxAltitude
        ...
        for (let i = 1; i <= numPointsElapsed; i++) {
            ...
            const altitude_i =
                -maxAltitude * Math.cos((2 * Math.PI * azimuth_i) / 360)
        ...
        for (let i = 0; i <= numPoints; i++) {
            ...
            const altitude =
                -maxAltitude * Math.cos((2 * Math.PI * azimuth) / 360)
    ...
    function buildSuncardHTML(me) {
        ...
        return (
            ...
            '<svg id="mySunSvg-' +
            idx +
            '" class="suncard-svg" viewBox="0 0 ' +
            SVG_WIDTH +
            ' ' +
            SVG_HEIGHT +
            '" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Sun path visualization"></svg>' +
            '<div class="suncard-events">' +
            '<div class="suncard-event">' +
            '<div class="suncard-event-label">Dawn</div>' +
            '<div class="suncard-event-time" id="dawnTime-' +
            idx +
            '">--:--</div>' +
            '</div>' +
            '<div class="suncard-event">' +
            '<div class="suncard-event-label">Solar noon</div>' +
            '<div class="suncard-event-time" id="solarnoonTime-' +
            idx +
            '">--:--</div>' +
            '</div>' +
            '<div class="suncard-event">' +
            '<div class="suncard-event-label">Dusk</div>' +
            '<div class="suncard-event-time" id="duskTime-' +
            idx +
            '">--:--</div>' +
            '</div>' +
            '</div>' +
    ...
    function draw_card(me) {
        ...
        document.getElementById('solarnoonTime-' + idx).textContent =
            formatTime(solarEvents.solarnoon)
        document.getElementById('currentTime-' + idx).textContent = formatTime(
            new Date(),
        )
    ...
    return {
        ...
        run: function (me) {
            ...
            Domoticz.request('type=command&param=getsettings', true).then(
                function (response) {
                    if (response && response.status === 'OK') {
                        LATITUDE =
                            parseFloat(response.Location['Latitude']) || 0
                    }
                },
            )
